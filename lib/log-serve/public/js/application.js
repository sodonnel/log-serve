...
jQuery(function($) {
	...
	var filekey   = $('#container').attr("data-filekey")
	visible_line = first_visible_logline()
	start_pos    = Number($(visible_line).attr('sp'))
	last_set     = Number($('#container').attr('lastSetPosition'));
	if (last_set && last_set == start_pos) {
	    start_pos = Number($(visible_line).attr('ep'))
	}
	
	$.post("/file/"+filekey+"/search", $("#search").serialize()+'&'+$.param({ 'position': start_pos }), function(response) {
	    eval(response);
	}).fail(function() {
...
function scroll_to_position(pos, highlight) {
	...
	if (Number($(this).attr('sp')) >= pos) {
	    ...
	    $(scrollTopSelector).scrollTop($(scrollTopSelector).scrollTop() - 65);
	    if (highlight == true) {
